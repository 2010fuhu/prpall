export default { 
    ...
    methods:{
      show() {
       ...
       initMainConstructData(obj){
        ...
        this.startFixDateOrg=obj.constructInfoVo.startFixDate
        this.endFixDateOrg=obj.constructInfoVo.endFixDate
    
        this.$nextTick(()=>{
          this.$uiCommon.setTitle(this.$refs.Construct.children)
          this.$uiCommon.addevent(this,'constructAddress')
          this.$uiCommon.addevent(this,'constructName')
          //this.$uiCommon.addevent(this,'endFixDate')
          //this.$uiCommon.addevent(this,'startFixDate')
          this.$uiCommon.addevent(this,'addAssureMonth')
        })
      }, 
      addassuremonth()
      {
      ...
      if(this.constructInfoVo.addAssureMonth!=this.$refs.addAssureMonth.title){
          this.$refs.addAssureMonth.title.style.backgroundColor='FFFF00'
      }else{
          this.$refs.addAssureMonth.title.style.backgroundColor=''
      }
      },
    ...
    changeCorlor(){
      this.$nextTick(()=>{
        if(this.endFixDateOrg!=this.constructInfoVo.endFixDate){
          for(let i=0;i<this.$refs.endFixDate.$el.children.length;i++){
            this.$refs.endFixDate.$el.children[i].style.backgroundColor="#FFFF00"
          }
        }else{
          for(let i=0;i<this.$refs.endFixDate.$el.children.length;i++){
            this.$refs.endFixDate.$el.children[i].style.backgroundColor=""
          }
        }
        if(this.startFixDateOrg!=this.constructInfoVo.startFixDate){
          for(let i=0;i<this.$refs.startFixDate.$el.children.length;i++){
            this.$refs.startFixDate.$el.children[i].style.backgroundColor="#FFFF00"
          }
        }else{
          for(let i=0;i<this.$refs.startFixDate.$el.children.length;i++){
            this.$refs.startFixDate.$el.children[i].style.backgroundColor=""
          }
        }
      })
    },
