  ...
  const IdentifyTypeArray=[
     ...
     export default  ({
		  ...
		  watch:{
			...
			'insuredShow.customerCName'(){
				this.setColor('customerCName')
			},
			'insuredShow.identifyType'(){
				this.setColor('identifyType')
			},
			'insuredShow.insuredType'(){
				this.setColor('insuredType')
			}			
				...
				if(this.$store.state.initStatus==1){
					if(this.insuredShow.flag=='I'){
						...
						if(this.insuredShow[key]!=insuredInfoListOrg[this.currentPage][key]){
							...
							this.$refs[key].className='common'
						}
					}
				}	
			},
